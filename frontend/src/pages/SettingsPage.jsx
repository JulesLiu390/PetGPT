...
const SettingsPage = () => {
  ...
  // 当同步设置加载完成时，初始化本地设置
  useEffect(() => {
    if (!settingsLoading && syncedSettings) {
      // 合并默认设置和实际设置
      setLocalSettings({ ...defaultSettings, ...syncedSettings });
    }
  }, [syncedSettings, settingsLoading]);

  // 当 assistants 加载完成后，如果没有设置默认 assistant，自动选择第一个
  useEffect(() => {
    if (localSettings && assistants.length > 0 && !localSettings.defaultAssistant) {
      setLocalSettings(prev => ({ ...prev, defaultAssistant: assistants[0].id }));
    }
  }, [assistants, localSettings?.defaultAssistant]);

  // 当 modelConfigs 加载完成后，如果没有设置默认 model，自动选择第一个
  useEffect(() => {
    if (localSettings && modelConfigs.length > 0 && !localSettings.defaultModelConfig) {
      setLocalSettings(prev => ({ ...prev, defaultModelConfig: modelConfigs[0].id }));
    }
  }, [modelConfigs, localSettings?.defaultModelConfig]);
