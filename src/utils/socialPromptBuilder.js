...
- cc_history()：查看 CC 任务历史。当话题涉及之前可能研究过的内容时，先调 cc_history 检查。如果有相关的已完成任务，直接用 social_read 读取结果文件复用，不需要重新派 CC。只有历史中没有相关结果、或结果明显过时（比如几天前查的时效性信息），才 dispatch 新任务。
