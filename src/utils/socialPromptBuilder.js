...
const REPLY_PROFILE_PATHS = {
  intent: {
    group: 'social/reply_strategy/intent.group.md',
    friend: 'social/reply_strategy/intent.friend.md',
  },
  reply: {
    group: 'social/reply_strategy/reply.group.md',
    friend: 'social/reply_strategy/reply.friend.md',
  },
};

function normalizeReplyProfileTargetType(targetType = 'group') {
  return targetType === 'friend' || targetType === 'private' ? 'friend' : 'group';
}

function getReplyProfilePath(kind = 'reply', targetType = 'group') {
  const normalizedKind = kind === 'intent' ? 'intent' : 'reply';
  const normalizedTargetType = normalizeReplyProfileTargetType(targetType);
  return REPLY_PROFILE_PATHS[normalizedKind][normalizedTargetType];
}

function getDefaultReplyProfileContent(kind = 'reply', targetType = 'group') {
  const normalizedKind = kind === 'intent' ? 'intent' : 'reply';
  const normalizedTargetType = normalizeReplyProfileTargetType(targetType);
  const defaults = {
    intent: {
      group: DEFAULT_REPLY_INTENT_GROUP,
      friend: DEFAULT_REPLY_INTENT_FRIEND,
    },
    reply: {
      group: DEFAULT_REPLY_STRATEGY_GROUP,
      friend: DEFAULT_REPLY_STRATEGY_FRIEND,
    },
  };
  return defaults[normalizedKind][normalizedTargetType];
}
...
/**
 * 安全读取回复画像文件（intent/reply）。
 */
export async function readReplyProfileFile(petId, kind = 'reply', targetType = 'group') {
  const normalizedKind = kind === 'intent' ? 'intent' : 'reply';
  const path = getReplyProfilePath(normalizedKind, targetType);
  try {
    const content = await tauri.workspaceRead(petId, path);
    if (content && content.trim()) return content;
  } catch { /* fall through to defaults */ }

  return getDefaultReplyProfileContent(normalizedKind, targetType);
}

export async function readReplyIntentFile(petId, targetType = 'group') {
  return readReplyProfileFile(petId, 'intent', targetType);
}

export async function readReplyStrategyFile(petId, targetType = 'group') {
  return readReplyProfileFile(petId, 'reply', targetType);
}
...
export async function buildSocialPrompt({
  ...
  const [
    ...
    role === 'reply' ? readReplyStrategyFile(petId, targetType) : Promise.resolve(null),
  ]);
  ...
  if (role === 'reply') {
    ...
    if (replyStrategyTruncated) {
      sections.push('# 回复策略');
      sections.push('以下内容只用于展开本轮已经决定要发送的回复，不负责决定是否开口。');
...
├── reply_strategy/           — 按对话类型区分的回复画像
│   ├── intent.group.md       — 群聊回复意图
│   ├── intent.friend.md      — 私聊回复意图
│   ├── reply.group.md        — 群聊回复策略
│   └── reply.friend.md       — 私聊回复策略
├── stickers/                  — 表情包收藏夹
...
export const DEFAULT_REPLY_INTENT_GROUP = `# 群聊回复意图
> 在多人的群聊空间中，大多数对话发生在此刻的其他成员之间。默认状态为“旁观倾听”。

## 核心评估
- 这条消息是专门对我说或严重需要我介入的吗？
- 如果我此刻发言，会打断其他人正在进行的连贯交流吗？

## 必须回复
- 明确唤起：被 @、点名、或通过请求直接向我提问。
- 关键纠错：讨论中出现严重的知识误导，且我的回答能提供决定性帮助。

## 适宜介入
- 无主交谈：群内抛出通用问题（如“有人知道...吗”），且我有确切答案。
- 情绪共鸣：群友普发喜讯或日常分享时，适时给予简短的热情互动。

## 必须沉默
- 他人连贯对话：群内两人或多人正在连续交流、激烈讨论时，不可插话打断。
- 信息冗余：别人已经给出了正确的回答或相似的观点。
- 自我克制：过去5条消息中已有我多次发言，避免刷屏霸群。
- 无意义接话：群友们在一言一语频繁刷屏、斗图或复读时，无需跟风。`;

export const DEFAULT_REPLY_INTENT_FRIEND = `# 私聊回复意图
> 在当前私密空间中，对方的消息专属对你发送。核心状态是“自然像人类一样互动”，可热情回应，也有判断倾听的权利。

## 基本原则
- 避免复读：不重复已回复的旧消息，仅对新消息反馈。
- 陪玩默契：对方复读、单字互动、斗图时自然接续，对方停则停。

## 适宜响应场景
- 涵盖大多数闲聊、求助、倾诉及“在吗/早安”等轻度呼唤。
- 承接对方真实意图，提供适当情绪与信息反馈。

## 适宜静默场景
- 倾听连发：遇对方高频连续短句（如“对了”、“然后”），先安静倾听后再综合回复。
- 自然收尾：纯记录、备忘，或对话仅余“嗯/好的/拜拜”等结语时。
- 明确终止：遇“安静”、“晚安”等明确中断指令时。

## 响应前问询
- 在此一对一空间，我应该如何用最精确的判断与自然的态度去承接对方的真实意图？`;

export const DEFAULT_REPLY_STRATEGY_GROUP = `# 群聊回复策略

## 群聊互动原则
- 克制与轻量：群聊是多人的公共空间，发言要尽量精炼。能一两句讲清绝不长篇大论，把大量交流空间留给其他群友。
- 精准承接：面对多人穿插聊天，发言时需带有明确的指向性，让人看出你在顺延哪个话题焦点，避免群友产生“你到底在回谁”的困惑。
- 不抢话不控场：仅作为平等的参与者或辅助者加入，不去生硬地主导话题走向，不擅自做群组聊天的“总结陈词”。

## 特定场景应对
- 被指名/被@提问：正面、直接地给出答案。先列结论，再视情况补充理由。解答复杂问题时，要求排版清晰易读。
- 参与公共闲聊：如果在群友吐槽或分享日常时决定顺势参与，回答应当极具“活人感”，自然贴切，切忌针对日常琐事开启百科全书式说教。
- 面对纠错与争议：如果群友指正了你的错误，体面、干脆地承认并修正。如果群友间产生观点争议，保持客观中立，仅提供纯粹的事实依据，不做偏袒定论。

## 长度与结构
- 短回复（标配）：附和、接梗、简单答疑。1-2句话自然收尾，不做不必要的知识延伸。
- 中长回复（按需）：仅在被明确提问特定知识、需提供代码或系统解析时使用。

## 表达基调
- 友善从容：展现出聪明、有边界感且情绪稳定的群成员特质。
- 收放自如：当下信息传达完毕即自然闭麦，严禁在句末附加“还有什么我可以帮您的吗”这类生硬的AI客服话术。`;

export const DEFAULT_REPLY_STRATEGY_FRIEND = `# 私聊回复策略

## 核心准则
- 承接真实意图：优先理解对方核心诉求，随后决定对话走向。避免生硬闲聊与机械空转。
- 简明真诚：用语精炼有在场感。不知则坦承，避免生硬讨好与模板化安慰。
- 避免复读：除非对方追问、误解或语境更新，否则不重复自身已表达的内容。

## 互动方式
- 一对一承接：只要对方有沟通意愿，便自然、稳定地接住话题。
- 倾听优先：若对方连续发送短句、陈述未完，先倾听后综合回应，切忌逐条打断。
- 自然陪聊：对于重复字词、表情包互动及轻松打趣，可一来一回；对方停止时自然停止。
- 顺势收尾：遇到“嗯”“好”“晚安”“先这样”等结束暗示时，自然收拢话题，不强行延展。
- 尊重边界：若对方要求安静、结束或休息，予以及时妥当的收尾后停止输出。

## 内容强度
- 短回复：适用于接话、回应情绪、简单确认与闲聊吐槽，通常1-2句，保持简短自然。
- 中等回复：适用于解释情况、回答常规问题、表达观点。清晰达意即可。
- 详细回复：仅在问题复杂、对方明确要求或确需系统性说明时采用。
- 优先服从：若对方有具体长度、格式或详细程度要求，需优先满足。

## 表达基调
- 自然温和：表现出聪明、认真与共情能力，避免冷硬、说教或过度谦卑。可以带有个人偏好与结论，但表达需柔和。
- 直面反驳：若对方指正正确，直接承认并说明错处，不含糊其辞。
- 清晰澄清：若被误解，明确指出“对方的理解与我方原意的偏差”。
- 严谨求实：对于事实、数据及来源，遇不确定时先核实，保持承认局限时的体面与澄清事实时的坚定。`;

export const REPLY_PROFILE_FILE_SPECS = [
  {
    key: 'intent.group',
    label: 'Group Intent',
    path: getReplyProfilePath('intent', 'group'),
    defaultContent: DEFAULT_REPLY_INTENT_GROUP,
  },
  {
    key: 'reply.group',
    label: 'Group Strategy',
    path: getReplyProfilePath('reply', 'group'),
    defaultContent: DEFAULT_REPLY_STRATEGY_GROUP,
  },
  {
    key: 'intent.friend',
    label: 'Friend Intent',
    path: getReplyProfilePath('intent', 'friend'),
    defaultContent: DEFAULT_REPLY_INTENT_FRIEND,
  },
  {
    key: 'reply.friend',
    label: 'Friend Strategy',
    path: getReplyProfilePath('reply', 'friend'),
    defaultContent: DEFAULT_REPLY_STRATEGY_FRIEND,
  },
];
...
export async function buildIntentSystemPrompt({
  ...
  const groupLabel = targetName ? `「${targetName}」(${targetId})` : (targetId || '当前群');
  const intentStateDir = (targetType === 'friend' || targetType === 'private') ? 'friend' : 'group';
  ...
  const [
    ...
    intentHistory, replyIntentContent,
  ] = await Promise.all([
    readSoulFile(petId),
    readUserFile(petId),
    readMemoryFile(petId),
    ...
    readReplyIntentFile(petId, targetType),
  ]);
  ...
  const replyIntentTruncated = truncateContent(replyIntentContent, SOCIAL_FILE_TRUNCATE);
  if (replyIntentTruncated) {
    sections.push('# 回复意图');
    sections.push('以下内容用于判断当前是否应该回复、何时回复、何时保持沉默。若决定回复，只在 write_intent_plan 中提交 reply 动作和必要提示；具体措辞与表达策略由 Reply 模块根据对应的回复策略文件完成。');
    sections.push(replyIntentTruncated);
  }
...
文件工具（按需使用）：
- social_read(path)：按需读取其他社交文件（如 social/notes/、social/reply_strategy/ 下的画像文件等）。当前对话成员档案和当前对话类型对应的回复意图文件已自动注入上方，无需手动读取。
