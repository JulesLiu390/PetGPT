...
impl McpClient {
    ...
    pub async fn connect(&self) -> Result<(), String> {
        ...
        *self.stdin_tx.lock().unwrap() = Some(stdin_tx);
            ...
            rt.block_on(async {
                while let Some(msg) = stdin_rx.recv().await {
                    if let Err(e) = stdin.write_all(msg.as_bytes()) {
                        log::error!("[MCP][{}] Failed to write to stdin: {}", server_name_clone, e);
                        break;
                    }
                    if let Err(e) = stdin.flush() {
                        log::error!("[MCP][{}] Failed to flush stdin: {}", server_name_clone, e);
                        break;
                    }
                }
            });
        ...
        let capabilities_clone = self.server_capabilities.clone();
                                ...
                                if let Ok(notif) = serde_json::from_str::<JsonRpcNotification>(trimmed) {
                                    match notif.method.as_str() {
                                        "notifications/tools/list_changed" => {
                                            ...
                                            // Note: We can't refresh tools here synchronously
                                            // The manager should handle this via events
                                        }
                                    ...
                                    log::debug!("[MCP][{}] Unknown message: {}", server_name_stdout, trimmed);
                                }
                            }
                        }
                    }
                    Err(e) => {
                        log::error!("[MCP][{}] Error reading stdout: {}", server_name_stdout, e);
                        break;
                    }
                }
            }
