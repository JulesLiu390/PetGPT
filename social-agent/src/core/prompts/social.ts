/**
 * socialPromptBuilder.ts — Direct port of src/utils/socialPromptBuilder.js (Tauri side).
 *
 * Factory: createSocialPromptBuilders(platform) returns the bound readers + the
 * two main prompt builders (buildSocialPrompt for Reply mode, buildIntentSystemPrompt
 * for Intent mode).
 *
 * Prompt strings are preserved byte-for-byte from the original — these are
 * load-bearing LLM behaviour spec.
 */
import type { Platform } from '../../platform/index.ts';
import { createPetFileReaders, truncateContent } from './petFiles.ts';
import { formatCurrentTime } from './time.ts';

// ─────────────────── constants ───────────────────

/** social file max chars (was imported from socialToolExecutor in original) */
const SOCIAL_FILE_MAX_CHARS = 20000;
...
export type SocialRole = 'observer' | 'reply';
export type TargetType = 'group' | 'friend';

export interface IntentActionLite {
  type?: string;
  id?: string | number;
  file?: string;
  atTarget?: string;
...
export interface IntentHistoryEntry {
  ts?: string | number;
  actions?: IntentActionLite[];
  briefDigest?: string;
}

export interface SubagentRegistryEntry {
  target: string;
  status: 'done' | 'running' | 'timeout' | 'failed';
  task: string;
  outputPath?: string;
  createdAt: number;
  readByIntent?: boolean;
  error?: string;
}

export type SubagentRegistry = Map<string, SubagentRegistryEntry>;
...
export interface BuildSocialPromptOptions {
  petId: string;
...
export interface SocialPromptBuilders {
  // readers
  readSocialMemoryFile:    (petId: string) => Promise<string | null>;
  readGroupRuleFile:       (petId: string, targetId: string) => Promise<string | null>;
  readPeopleCacheFile:     (petId: string, targetId: string, targetType?: TargetType) => Promise<string | null>;
  readIntentHistoryFile:   (petId: string, targetId: string, targetType?: TargetType, limit?: number) => Promise<IntentHistoryEntry[]>;
  readReplyBriefFile:      (petId: string, targetId: string, targetType?: TargetType) => Promise<string | null>;
  readScratchNotesFile:    (petId: string, targetId: string, targetType?: TargetType) => Promise<string | null>;
  readIntentStateFile:     (petId: string, targetId: string, targetType?: TargetType) => Promise<string | null>;
  readContactsFile:        (petId: string) => Promise<string | null>;
  readStickerIndexForPrompt: (petId: string) => Promise<string | null>;
  readReplyStrategyFile:   (petId: string) => Promise<string | null>;
  // builders
  buildSocialPrompt:       (opts: BuildSocialPromptOptions) => Promise<string>;
  buildIntentSystemPrompt: (opts: BuildIntentPromptOptions) => Promise<string>;
}

// ─────────────────── pure helpers (no platform needed) ───────────────────

export function buildSubagentStatusSection(
  subagentRegistry: SubagentRegistry | null | undefined,
  targetId: string,
): string {
...
export function formatIntentHistoryForPrompt(entries: IntentHistoryEntry[]): string {
  if (!entries || entries.length === 0) return '';
  const lines = ['# 最近 Intent 行动记录（客观历史，仅供防重复参考）'];
  for (const e of entries) {
    const hhmm = (() => {
      try {
        const d = new Date(e.ts as any);
...
// ─────────────────── DEFAULT_REPLY_STRATEGY (verbatim) ───────────────────
...
export function createSocialPromptBuilders(platform: Platform): SocialPromptBuilders {
  const { readSoul, readUser, readMemory } = createPetFileReaders(platform);
  const ws = platform.workspace;

  // Common safe-read helper
  async function safeRead(petId: string, path: string): Promise<string | null> {
    try {
      const content = await ws.read(petId, path);
      return content || null;
    } catch {
      return null;
    }
  ...
  async function readSocialMemoryFile(petId: string): Promise<string | null> {
    return safeRead(petId, 'social/SOCIAL_MEMORY.md');
  }

  async function readGroupRuleFile(petId: string, targetId: string): Promise<string | null> {
    if (!targetId) return null;
    return safeRead(petId, `social/group/RULE_${targetId}.md`);
  }

  async function readPeopleCacheFile(
    petId: string, targetId: string, targetType: TargetType = 'group',
  ): Promise<string | null> {
    if (!targetId) return null;
    const dir = targetType === 'friend' ? 'friend' : 'group';
    return safeRead(petId, `social/${dir}/PEOPLE_CACHE_${targetId}.md`);
  }

  async function readIntentHistoryFile(
    petId: string, targetId: string, targetType: TargetType = 'group', limit = 5,
  ): Promise<IntentHistoryEntry[]> {
    if (!targetId) return [];
    const dir = targetType === 'friend' ? 'friend' : 'group';
    try {
      const raw = await ws.read(petId, `social/${dir}/scratch_${targetId}/intent_history.jsonl`);
      if (!raw || !raw.trim()) return [];
      const lines = raw.split('\n').filter(l => l.trim()).slice(-limit);
      return lines
        .map(line => { try { return JSON.parse(line) as IntentHistoryEntry; } catch { return null; } })
        .filter((x): x is IntentHistoryEntry => x !== null);
    } catch {
      return [];
    }
  }

  async function readReplyBriefFile(
    petId: string, targetId: string, targetType: TargetType = 'group',
  ): Promise<string | null> {
    if (!targetId) return null;
    const dir = targetType === 'friend' ? 'friend' : 'group';
    return safeRead(petId, `social/${dir}/scratch_${targetId}/reply_brief.md`);
  }

  async function readScratchNotesFile(
    petId: string, targetId: string, targetType: TargetType = 'group',
  ): Promise<string | null> {
    if (!targetId) return null;
    const dir = targetType === 'friend' ? 'friend' : 'group';
    return safeRead(petId, `social/${dir}/scratch_${targetId}/notes.md`);
  }

  async function readIntentStateFile(
    petId: string, targetId: string, targetType: TargetType = 'group',
  ): Promise<string | null> {
    if (!targetId) return null;
    const dir = targetType === 'friend' ? 'friend' : 'group';
    return safeRead(petId, `social/${dir}/INTENT_${targetId}.md`);
  }

  async function readContactsFile(petId: string): Promise<string | null> {
    return safeRead(petId, 'social/CONTACTS.md');
  }

  async function readStickerIndexForPrompt(petId: string): Promise<string | null> {
    try {
      const content = await ws.read(petId, STICKER_INDEX_PATH);
  ...
  async function readReplyStrategyFile(petId: string): Promise<string | null> {
    return safeRead(petId, 'social/REPLY_STRATEGY.md');
  }

  // ─── Guidance helpers (closure over readers above) ───

  function contactsGuidance(content: string | null): string {
    if (!content) {
  ...
  function socialMemoryGuidance(content: string | null, targetName: string, targetId: string): string {
  ...
  function groupRuleGuidance(content: string | null, targetName: string, targetId: string): string {
    const groupLabel = targetName ? `「${targetName}」(${targetId})` : targetId;
  ...
  // ─── Mode / instruction builders (pure-fn-ish, no I/O) ───

  function buildSocialModeInstruction(targetName: string, targetId: string, botQQ: string): string {
...
- **@人格式**：需要 @ 某人时，必须用 @QQ号 的格式（例如 @123456789），不要用 @昵称。`;
  ...
  function buildLurkObservationInstruction(targetName: string, targetId: string, botQQ: string): string {
    const target = targetName && targetId ? `"${targetName}"（${targetId}）` : targetName ? `"${targetName}"` : '一个聊天';
    const qqInfo = botQQ ? `你的 QQ 号是 ${botQQ}。` : '';
...
你是一个安静但勤奋的观察者。把精力放在维护群档案、人物档案和社交记忆上，而不是回复。`;
  ...
  function buildLurkToolInstruction(targetName: string, targetId: string): string {
    const groupLabel = targetName ? `「${targetName}」(${targetId})` : targetId;
...
观察完毕后，输出"[沉默]"。不要输出任何其他纯文本。`;
  ...
  function buildReplyToolInstruction(targetName: string, targetId: string): string {
    const groupLabel = targetName ? `「${targetName}」(${targetId})` : targetId;
...
⚠️ 【再次提醒】想说话 → 必须调用 send_message 工具。直接输出纯文本群友看不到。发送前先回顾上方 assistant 消息，确认没有重复。如果已经说过类似的话，输出"[沉默]：<理由>"。`;
  ...
  async function buildSocialPrompt(opts: BuildSocialPromptOptions): Promise<string> {
    const {
      petId,
      socialPersonaPrompt = '',
      // atMustReply = true,           // unused in original body, retained in API
      targetName = '',
      targetId = '',
      targetType = 'group',
      botQQ = '',
      ...
      msgDelimiterL = '',
      msgDelimiterR = '',
      // agentCanEditStrategy = false, // unused in original body
      lurkMode = 'normal',
      role = 'reply',
      // intentPlan = null,             // unused in original body
    } = opts;
    ...
    sections.push(groupRuleTruncated || '（空）');
    if (role === 'observer') {
    ...
    if (role === 'observer') {
      ...
      sections.push('# 联系人索引');
      sections.push(contactsTruncated || '（空）');
    ...
    if (role === 'observer') {
      ...
      sections.push(socialMemoryTruncated || '（空）');
  ...
  // ─── buildIntentSystemPrompt (Intent mode) ───

  async function buildIntentSystemPrompt(opts: BuildIntentPromptOptions): Promise<string> {
    const {
    ...
    const [
      ...
      (async () => {
        const dir = targetType === 'friend' ? 'friend' : 'group';
        try {
          const raw = await ws.read(petId, `social/${dir}/scratch_${targetId}/lessons.json`);
          if (!raw || !raw.trim()) return null;
          const lessons = JSON.parse(raw) as Array<{ count?: number; problem?: string; action?: string; lesson?: string }>;
      ...
      (async () => {
        const dir = targetType === 'friend' ? 'friend' : 'group';
        try { return await ws.read(petId, `social/${dir}/scratch_${targetId}/principles.md`); } catch { return null; }
      })(),
...
`);
    ...
    // 工具说明 (large block — verbatim from original)
    sections.push(`# 可用工具
...
- 错了就认：无论你的人设多傲娇/多自信，事实错误必须承认。大方说"搞错了，谢谢纠正"比死撑强一万倍。真正的自信是敢认错，死不认错是心虚`);
  ...
  return {
    readSocialMemoryFile,
    readGroupRuleFile,
    readPeopleCacheFile,
    readIntentHistoryFile,
    readReplyBriefFile,
    readScratchNotesFile,
    readIntentStateFile,
    readContactsFile,
    readStickerIndexForPrompt,
    readReplyStrategyFile,
    buildSocialPrompt,
